    ...
    async def Feed(self):
        ...
        while True:
            response = await get.RequestUrl(
                self.config, self.init, headers=[("User-Agent", self.user_agent)]
            )
    ...
    async def tweets(self):
        ...
        for tweet in self.feed:
            ...
            try:
                ...
                tweet_dict["avatar"] = tweet.find("td", {"class": "avatar"}).find(
                    "img"
                )["src"]
...
def Lookup(config):
    ...
    try:
        if config.User_id is not None:
            ...
            config.Username = get_event_loop().run_until_complete(
                get.Username(config.User_id)
            )

        url = f"https://mobile.twitter.com/{config.Username}?prefetchTimestamp=" + str(
            int(time.time() * 1000)
        )
        get_event_loop().run_until_complete(
            get.User(url, config, db.Conn(config.Database))
        )
        ...
        if "no current event loop" in str(e):
            logme.exception(
                __name__ + ":Lookup:Previous attempt to to create an event loop failed."
            )
